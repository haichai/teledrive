        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="font-black text-lg tracking-tight text-slate-900 dark:text-white">TeleDrive</span>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              {lang === 'vi' ? 'Lưu trữ đám mây qua Telegram MTProto không giới hạn' : 'Unlimited cloud storage powered by Telegram MTProto'}
            </p>
          </div>
        </div>
              ...
              {showAdvancedApi && (
                <div className="mt-3 space-y-2.5 pb-2 text-xs">
                  <p className="text-[11px] text-slate-500 dark:text-slate-400">
                    {lang === 'vi'
                      ? 'Mặc định TeleDrive sử dụng API MTProto chính thức có sẵn. Nếu muốn dùng App riêng từ my.telegram.org, hãy điền bên dưới:'
                      : 'By default TeleDrive uses official MTProto credentials. Enter below only if using your own my.telegram.org app:'}
                  </p>
                  <div>
                    <input
                      type="text"
